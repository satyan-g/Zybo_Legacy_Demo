# build_sw.py — Build bare-metal AXI GPIO demo for Zynq using Vitis 2024.2
# Run: vitis -s scripts/05_axi_gpio/build_sw.py
...
out_dir = os.path.join(proj_root, "output/05_axi_gpio")
xsa_file = os.path.join(out_dir, "system.xsa")
sw_dir = os.path.join(out_dir, "vitis_workspace")
app_src = os.path.join(proj_root, "sw/05_axi_gpio/axi_gpio_demo.c")

APP_NAME = "axi_gpio_demo"

print("=" * 44)
print(" Building: 05_axi_gpio Software")
...
# Copy our source
shutil.copy2(app_src, os.path.join(app_src_dir, "axi_gpio_demo.c"))
...
if found_elf:
    ...
    print(f" ELF: output/05_axi_gpio/{APP_NAME}.elf")
    print("=" * 44)
else:
