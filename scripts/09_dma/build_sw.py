# build_sw.py — Build bare-metal DMA demo for Zynq using Vitis 2024.2
# Run: vitis -s scripts/09_dma/build_sw.py
...
out_dir = os.path.join(proj_root, "output/09_dma")
xsa_file = os.path.join(out_dir, "system.xsa")
sw_dir = os.path.join(out_dir, "vitis_workspace")
app_src = os.path.join(proj_root, "sw/09_dma/dma_demo.c")

APP_NAME = "dma_demo"

print("=" * 44)
print(" Building: 09_dma Software")
...
shutil.copy2(app_src, os.path.join(app_src_dir, "dma_demo.c"))
...
if found_elf:
    ...
    print(f" ELF: output/09_dma/{APP_NAME}.elf")
    print("=" * 44)
else:
