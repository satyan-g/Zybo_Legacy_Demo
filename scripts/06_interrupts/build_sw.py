# build_sw.py — Build bare-metal interrupt demo for Zynq using Vitis 2024.2
# Run: vitis -s scripts/06_interrupts/build_sw.py
...
out_dir = os.path.join(proj_root, "output/06_interrupts")
xsa_file = os.path.join(out_dir, "system.xsa")
sw_dir = os.path.join(out_dir, "vitis_workspace")
app_src = os.path.join(proj_root, "sw/06_interrupts/interrupt_demo.c")

APP_NAME = "interrupt_demo"

print("=" * 44)
print(" Building: 06_interrupts Software")
...
shutil.copy2(app_src, os.path.join(app_src_dir, "interrupt_demo.c"))
...
if found_elf:
    ...
    print(f" ELF: output/06_interrupts/{APP_NAME}.elf")
    print("=" * 44)
else:
