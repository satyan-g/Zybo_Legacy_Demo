# build_sw.py — Build bare-metal audio demo for Zynq using Vitis 2024.2
# Run: vitis -s scripts/11_audio/build_sw.py
...
out_dir = os.path.join(proj_root, "output/11_audio")
xsa_file = os.path.join(out_dir, "system.xsa")
sw_dir = os.path.join(out_dir, "vitis_workspace")
app_src = os.path.join(proj_root, "sw/11_audio/audio_demo.c")

APP_NAME = "audio_demo"

print("=" * 44)
print(" Building: 11_audio Software")
...
shutil.copy2(app_src, os.path.join(app_src_dir, "audio_demo.c"))
...
if found_elf:
    ...
    print(f" ELF: output/11_audio/{APP_NAME}.elf")
    print("=" * 44)
else:
