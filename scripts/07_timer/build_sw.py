# build_sw.py — Build bare-metal timer demo for Zynq using Vitis 2024.2
# Run: vitis -s scripts/07_timer/build_sw.py
...
out_dir = os.path.join(proj_root, "output/07_timer")
xsa_file = os.path.join(out_dir, "system.xsa")
sw_dir = os.path.join(out_dir, "vitis_workspace")
app_src = os.path.join(proj_root, "sw/07_timer/timer_demo.c")

APP_NAME = "timer_demo"

print("=" * 44)
print(" Building: 07_timer Software")
...
shutil.copy2(app_src, os.path.join(app_src_dir, "timer_demo.c"))
...
if found_elf:
    ...
    print(f" ELF: output/07_timer/{APP_NAME}.elf")
    print("=" * 44)
else:
