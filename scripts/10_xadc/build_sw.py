# build_sw.py — Build bare-metal XADC demo for Zynq using Vitis 2024.2
# Run: vitis -s scripts/10_xadc/build_sw.py
...
out_dir = os.path.join(proj_root, "output/10_xadc")
xsa_file = os.path.join(out_dir, "system.xsa")
sw_dir = os.path.join(out_dir, "vitis_workspace")
app_src = os.path.join(proj_root, "sw/10_xadc/xadc_demo.c")

APP_NAME = "xadc_demo"

print("=" * 44)
print(" Building: 10_xadc Software")
...
shutil.copy2(app_src, os.path.join(app_src_dir, "xadc_demo.c"))
...
if found_elf:
    ...
    print(f" ELF: output/10_xadc/{APP_NAME}.elf")
    print("=" * 44)
else:
